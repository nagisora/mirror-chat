...
- コミットメッセージは日本語です。
- 作業は細かく区切り、各作業が完了したらユーザーから明示されなくてもその作業単位で必ずコミットしてください。
- 変更作業はブランチ上で行い、ブランチがない場合は新しく作成してからコミットしてください。
- プルリクは `git push -u origin <branch>` の後に `gh pr create --base main --head <branch> --title "..." --body-file <file>` で作成してください。本文をコマンド引数へ直接埋め込まないでください。
- `gh pr edit` が `GraphQL: Projects (classic) ... (repository.pullRequest.projectCards)` で失敗する環境では、`gh api graphql` の `updatePullRequest` mutation で本文を更新してください。
