...
3. **API トークン**: Obsidian で設定したトークンを入力
4. **保存ルートパス**: `AI-Research`（Vault ルートからの相対フォルダ。例: `AI-Research` や `notes/AI-Research`）
5. 「保存」をクリック
...
5. 完了するとデスクトップ通知が表示され、Obsidian の Vault 内の `保存ルートパス/YYYY-MM-DD_質問の先頭20文字/` 以下に `question.md` や各 AI 名の `.md`、`Summary.md` が作成される
